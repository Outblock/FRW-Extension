...
interface Props extends Omit<SVGAttributes<SVGElement>, 'color'> {
  name: 'kucoin' | 'coinbase' | 'inbox' | 'account' | 'developer' | 'Subtract' | 'exec' | 'huobi' | 'plus' | 'kraken' | 'Binance' | 'chevron-right' | 'dollar' | 'currency' | 'security' | 'notification' | 'theme' | 'a-Group762' | 'Logo' | 'flowns' | 'backup' | 'search' | 'setting' | 'a-hamburgermenu' | 'dropdown' | 'a-close' | 'import' | 'back' | 'switch' | 'About' | 'create' | 'lock' | 'User-switch' | 'Address-book' | 'a-Vector11Stroke' | 'max' | 'Vector-2' | 'Vector' | 'wallet' | 'devices' | 'FUSD' | 'google-drive' | 'flow' | 'nfts' | 'copy' | 'checkmark' | 'close' | 'eye' | 'play-store' | 'app-store' | 'back-button';
  size?: number;
  color?: string | string[];
}
