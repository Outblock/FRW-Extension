...
const MoveEvm = (props: MoveBoardProps) => {
  ...
  const updateCurrentCollection = async () => {
    if (collectionList && cadenceNft) {
      ...
      const selectedShow = cadenceNft.find(
  ...
  const requestCadenceNft = async () => {
    ...
    const tokensWithNfts = cadenceResult.filter(
      (token) => token.nftIds && token.nftIds.length > 0
    );
  ...
  return (
    ...
    >
        ...
        {collectionDetail && (
          <Button onClick={() => setSelectCollection(true)}>
            ...
            <CardMedia
              ...
              image={
                'https://raw.githubusercontent.com/Outblock/Assets/main/ft/flow/logo.png'
              }
            />
            <CardMedia
              component="img"
        ...
        >
          {collectInfo.nfts.length > 0 ? (
            collectInfo.nfts.map((nfts) => (
              ...
              >
                <Button
                  onClick={() => toggleSelectNft(nfts.id)}
                  sx={{
                    padding: 0,
                    borderRadius: '16px',
                    backgroundColor: '#333',
            ...
            <Box sx={{ width: '100%', textAlign: 'center' }}>
              <Typography
                sx={{ color: '#FFFFFF66', fontSize: '14px', fontWeight: '700' }}
              >
                0 NFTs
              </Typography>
            </Box>
      ...
      <Button
        ...
        disabled={
          !collectionDetail ||
          (collectionDetail.nfts && collectionDetail.nfts.length === 0)
        }
        sx={{
          height: '50px',
          borderRadius: '12px',
          ...
          <>
            {failed ? (
              ...
              >
                ...
                {nftIdArray.length > 0 && nftIdArray.length} NFT
