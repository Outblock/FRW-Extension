...
const Nft = ({ data }) => {
  ...
  const claim = async (itemId, symbol) => {
    ...
    wallet.claimNFTFromInbox(userDomain, itemId, symbol).then(async (resp) => {
      setFailed(false);
      wallet.listenTransaction(resp, true, `${itemId} ${symbol} Claimed`, `You have claimed ${itemId} ${symbol} to ${userDomain}. \nClick to view this transaction.`);
      setIds('');
                            ...
                            return (
                              ...
                              >
                                <Box sx={{paddingRight:'0px',color:'#fff',width:'100%', display:'flex', justifyContent:'space-between'}}>
                                  <img src={fallback} style={{height: '100px', width: '100px', backgroundColor: '#282828', borderRadius: '12px'}}/>
                                </Box>
                                <Box 
                                  sx={{
                                    display:'flex',
