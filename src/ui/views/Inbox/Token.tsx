...
const Token = ({ data }) => {
  ...
  return (
    <ThemeProvider theme={theme}>
      <List
        sx={{ padding: '0 17px', marginTop: '19px', background: '#000000' }}
      >
        {!isLoading ? (
          <Box>
            {notEmpty ? (
              <>
                {(Object.keys(inbox['vaultBalances']) || []).map(
                  (ibx: any, v: any) => {
                    const contractName = ibx.split('.')[2];
                    const token = tokens.find(
                      (item) =>
                        item.contract_name.toLowerCase() ==
                        ...
                        {parseFloat(inbox['vaultBalances'][ibx]) > 0 && (
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                {ibx.split('.')[2] === 'FUSD'
                                  ? `$${parseFloat(inbox['vaultBalances'][ibx]).toFixed(2)}`
                                  : ''}
                              </Typography>
                            </Box>
                            <Box sx={{ flex: 1 }}></Box>
