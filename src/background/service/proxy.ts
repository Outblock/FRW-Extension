...
import {
  findAddressWithSeed,
  findAddressWithPK,
} from '@/ui/utils/modules/findAddressWithPK';
...
class Proxy {
  ...
  proxySignRequest = async (cadence, args) => {
    ...
    try {
      ...
      const result = await this.sendSignRequest(
        signwallet,
        topicId,
        cadence,
        args
  ...
  sendSignRequest = async (
    signwallet: SignClient,
    topic: string,
    cadence,
    args
  ) => {
  ...
  sendLoginRequest = async (signwallet: SignClient, topic: string) => {
    try {
      ...
      return await openapiService.loginV3(
        accountKey,
        deviceInfo,
        jsonObject.data.signature
      );
    } catch (error) {
      console.error('Error in first wallet request:', error);
