...
import { Box, Button, InputAdornment, Typography, Drawer, IconButton, TextField, ListItem, ListItemButton, Avatar, CardMedia } from '@mui/material';
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: '10px', px: '18px' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', height: '24px', margin: '20px 0 0', alignItems: 'center', }}>
        ...
        {collectionList.length > 0 &&
          <Box sx={{ display: 'flex', justifyContent: 'space-between', flexDirection: 'column', mb: '16px', mt: '16px', padding: '0' }}>
            <TextField
                  ...
                  <ListItemButton onClick={() => setSelected(obj.id)} sx={{ width: '100%', padding: '0', borderRadius: '12px','&:hover': { backgroundColor:'rgba(44, 44, 44, 0.1)'},}} >
                    <Box sx={{ backgroundColor: '#2C2C2C', width: "100%", height: '64px', display: 'flex', overflow: 'hidden', justifyContent: 'space-between', borderRadius: '12px','&:hover': { backgroundColor:'rgba(44, 44, 44, 0.5)'} }} key={index}>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', flex: '1', marginLeft: '16px', mt: '12px', height: '100%' }}>
                        ...
                        <Typography sx={{ fontSize: '12px', fontWeight: 400, color: '#FFFFFFCC' }}>{obj.NftCount} NFTs</Typography>
                      </Box>
