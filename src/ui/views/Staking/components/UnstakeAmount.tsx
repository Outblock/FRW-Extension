...
const UnstakeAmount = ({token, amount, setAmount, setSwapTypes, setError,removeError, coinInfo, nodeInfo}) => {
  ...
  useEffect(() => {
    if (coinInfo) {
      ...
      const staked = parseFloat(nodeInfo.tokensStaked)
      if (value > staked) {
        setError();
      } else {
        removeError();
      ...
      <Box sx={{
        ...
        <Box sx={{        
          ...
          >
            ...
            <Typography color="text.secondary" variant="caption">{nodeInfo.tokensStaked}</Typography>
            <Typography sx={{color:'#5E5E5E'}} variant="caption">Staked FLOW</Typography>
          </Box>
        </Box>
      </Box>
