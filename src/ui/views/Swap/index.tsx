  ...
  const updateCoinInfo = (token) => {
    if (selectTarget) {
      ...
      const coin = coinList.find(
        (coin) => coin.unit.toLowerCase() === token.symbol.toLowerCase()
      );
      if (coin) {
        setCoinInfo(coin);
        setToken0(token);
  ...
  const estimateOut = async () => {
    ...
    if (token0 && token1) {
      ...
      try {
        ...
        let price: any =
          result.data.tokenOutAmount / parseFloat(result.data.tokenInAmount);
        ...
        setAmount(
          (Math.round(result.data.tokenInAmount * 1000) / 1000).toFixed(4)
        );
      } catch (err) {
        setLoading(false);
  ...
  const estimate = async () => {
    ...
    if (token0 && token1) {
      ...
      try {
        ...
        setEstimateInfo(result.data);
        setOutAmount(
          (Math.round(result.data.tokenOutAmount * 1000) / 1000).toFixed(4)
        );
        setSwapTypes(0);
      } catch (err) {
        setLoading(false);
