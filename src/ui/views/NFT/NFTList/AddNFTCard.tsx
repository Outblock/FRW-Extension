...
const CollectionCard = ({ item, setAlertOpen, isLoading, onClick }: { item: CollectionItem, setAlertOpen: any, isLoading: boolean, onClick: any }) => {
    ...
    <Card 
      ...
      <CardActionArea
        sx={{
          ...
          height: '100%',
          padding: '12px 18px',
          background: 'linear-gradient(270deg, rgba(40, 40, 40, 0.32) 0%, rgba(40, 40, 40, 0.88) 30.56%, #282828 42.04%)',
      ...
      >
        <Box sx={{ display: 'flex', height: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box sx={{ display: 'flex', height: '100%', flexDirection: 'column', justifyContent: 'center' }}>
            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Box sx={{ marginLeft: '6px' }}><IconFlow size={14} /></Box>
              <ArrowForwardIcon sx={{ color: '#FFFFFF', fontSize: '12px', marginLeft: '5px' }} />
            </Box>
            {description && <Box sx={{ alignItems: 'center', height: '36px' }}>
              <Typography
                color="#5E5E5E"
                component="div"
          ...
          <IconButton
            onClick={() =>{if(!added && !isLoading)  onClick(item)}}
          >
            <Box                
              sx={{
                width: '32px',
              ...
              {isLoading ? <CircularProgress color="primary" size={24} /> :
                (added ? <DoneIcon sx={{ color: 'black', fontSize: '20px' }} /> :
                  <AddIcon color='primary' sx={{ fontSize: '20px' }} onClick={() => {
                    if(!added && !isLoading) {
                      setAlertOpen(true)}
                  }
                  }/> 
