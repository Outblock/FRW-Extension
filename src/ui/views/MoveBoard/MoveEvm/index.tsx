...
const MoveEvm = (props: MoveBoardProps) => {
  ...
  const updateCurrentCollection = async () => {
    if (collectionList && cadenceNft) {
      ...
      const cadenceResult =
        await usewallet.EvmNFTcollectionList(selectedCollection);
  ...
  const requestCadenceNft = async () => {
    ...
    const tokensWithNfts = cadenceResult.filter(
      (token) => token.ids && token.ids.length > 0
    );
    const filteredData = tokensWithNfts.filter(
      (item) => item.collection.flowIdentifier
    );
              ...
              collectInfo.nfts.map((items) => (
                <Box
                  ...
                  sx={{
                    ...
                    borderRadius: '16px',
                    marginBottom: '3px',
                    border: nftIdArray.includes(items.id)
                      ? '1px solid #41CC5D'
                      : 'none',
                  }}
                >
