...
export class PublicKeyCredentialRpEntity extends PublicKeyCredentialEntity {
  ...
  isEmpty() {
    return (this.name ?? this.id ?? undefined) === undefined;
  }
  toJSON() {
    return this.isEmpty()
      ? undefined
...
export class PublicKeyCredentialUserEntity extends PublicKeyCredentialEntity {
  ...
  isEmpty() {
    return (this.name ?? this.displayName ?? undefined) === undefined;
  }
  toJSON() {
    return this.isEmpty()
      ? undefined
...
export class PublicKeyCredentialCreationOptions {
  ...
  constructor(obj) {
    ...
    this.pubKeyCredParams = obj?.pubKeyCredParams?.map(
      (t) => new PublicKeyCredentialParameters(t)
    );
    this.timeout = obj?.timeout;
    this.excludeCredentials = obj?.excludeCredentials?.map(
      (t) => new PublicKeyCredentialDescriptor(t)
    );
...
export class PublicKeyCredentialRequestOptions {
  ...
  constructor(obj) {
    ...
    this.timeout = obj?.timeout;
    this.rpId = obj?.rpId;
    this.allowCredentials = obj?.allowCredentials?.map(
      (t) => new PublicKeyCredentialDescriptor(t)
    );
    this.userVerification = obj?.userVerification;
    this.extensions = obj?.extensions;
...
export class PublicKeyCredential extends Credential {
  ...
  constructor(obj) {
    ...
    if ('attestationObject' in (obj?.response ?? {}))
      this.response = new AuthenticatorAttestationResponse(obj?.response);
    if ('authenticatorData' in (obj?.response ?? {}))
      this.response = new AuthenticatorAssertionResponse(obj?.response);
  }
  toJSON() {
    return {
      id: this.id ?? undefined,
...
export class AuthenticatorData {
  ...
  toJSON() {
    return {
      ...
      attestedCredentialData: this.at
        ? this.attestedCredentialData?.toJSON()
        : undefined,
