...
const OnRampList = ({close}) => {
  ...
  const loadCoinbasePay = async () => {
    ...
    const onRampURL = generateOnRampURL({
      ...
      destinationWallets: [
        { address: address, blockchains: ['flow'] }
      ]
    });
  ...
  return(
    <Box>
      ...
      <Box sx={{display: 'flex', flexDirection: 'column', gap: '12px', mx: '18px'}}>
        <ButtonBase sx={{ width: '100%', backgroundColor: '#242424', borderRadius: '12px', height: '80px' }} onClick={loadMoonPay}>
          <img src={MoonPay} style={{ height: '40px' }}/>
        </ButtonBase>

        <ButtonBase sx={{ width: '100%', backgroundColor: '#0052FF', borderRadius: '8px', height: '80px' }} onClick={loadCoinbasePay}>
