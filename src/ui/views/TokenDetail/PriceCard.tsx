...
import {IconKraken, IconBinance, IconCoinbase, IconKucoin, IconHuobi} from '../../../components/iconfont';
...
const Binance = () => {
  return (
    <Box  sx={{display: 'flex', gap: '2px', alignItems: 'center'}}>
      <IconBinance size={20}/>
      <Typography variant="body1" color="text.secondary" sx={{fontWeight: 'medium', fontSize:'12px', alignSelf: 'center'}}>{chrome.i18n.getMessage('Binance')}</Typography>
    </Box>
  );
}
...
const Kraken = () => {
  return (
    <Box  sx={{display: 'flex', gap: '4px', alignItems: 'center'}}>
      <IconKraken size={15}/>
      <Typography variant="body1" color="text.secondary" sx={{fontWeight: 'medium', fontSize:'12px', alignSelf: 'center'}}>{chrome.i18n.getMessage('Kraken')}</Typography>
    </Box>
  );
}
...
const Coinbase = () => {
  return (
    <Box  sx={{display: 'flex', gap: '4px', alignItems: 'center'}}>
      ...
      <Typography variant="body1" color="text.secondary" sx={{fontWeight: 'medium', fontSize:'12px', alignSelf: 'center'}}>{chrome.i18n.getMessage('Coinbase')}</Typography>
    </Box>
  );
}
...
const Kucoin = () => {
  return (
    <Box  sx={{display: 'flex', gap: '4px', alignItems: 'center'}}>
      <IconKucoin size={15}/>
      <Typography variant="body1" color="text.secondary" sx={{fontWeight: 'medium', fontSize:'12px', alignSelf: 'center'}}>{chrome.i18n.getMessage('Kucoin')}</Typography>
    </Box>
  );
}
...
const Huobi = () => {
  return (
    <Box  sx={{display: 'flex', gap: '4px', alignItems: 'center'}}>
      <IconHuobi size={15}/>
      <Typography variant="body1" color="text.secondary" sx={{fontWeight: 'medium', fontSize:'12px', alignSelf: 'center'}}>{chrome.i18n.getMessage('Huobi')}</Typography>
    </Box>
  );
}
...
const PriceCard = ({token, price, setPrice, providers}) => {
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && data && payload && data.length > 0 && payload.length > 0) {
      ...
      return (
        <Box className="custom-tooltip" sx={{backgroundColor: 'neutral.main', borderRadius: '8px', padding: '5px', border: '1px solid #4C4C4C'}}>
          <Typography variant="body1" color='text.primary' sx={{fontWeight: 'medium', fontSize:'12px', alignSelf: 'center'}}>{'$'+data.price}</Typography>
          <Typography variant="body1" color='text.secondary' sx={{fontWeight: 'medium', fontSize:'10px', alignSelf: 'center'}}>{date.format( showHour ? 'YYYY-MM-DD hh:mm' : 'YYYY-MM-DD')}</Typography>
        </Box>
      );
    }
    ...
    <Box
      ...
      <Box sx={{display: 'flex',px: '18px', pt: '12px', pb:'4px', flexDirection: 'column'}}>
        <Box sx={{display: 'flex'}}>
          <Box sx={{display: 'flex', alignItems: 'baseline', gap: '2px', flexDirection: 'column',}}>
            <Typography variant="body1" sx={{fontSize:'16px', fontWeight: '600'}}>{chrome.i18n.getMessage('Recent_Price')}</Typography>
            <Box sx={{display: 'flex', gap: '8px', alignItem: 'center'}}>
              <Typography variant="body1" sx={{fontWeight: 'medium', fontSize:'16px', alignSelf: 'center'}}>{`$${price}`}</Typography>
              <Box sx={{display: 'flex', backgroundColor: isUp() ? '#182810' : '#271716', py: '1px', pl:'2px', pr: '6px', borderRadius: '6px', alignItem: 'center'}}>
                { isUp() ? <ArrowDropUpRoundedIcon color='success'/> : <ArrowDropDownRoundedIcon color='error'/> }
                <Typography variant="body1" color={isUp() ? 'success.main' : 'error.main'} sx={{fontWeight: 'medium', fontSize:'12px', alignSelf: 'center'}}>{`${change.toFixed(2)}%`}</Typography>
              </Box>
            </Box>
          </Box>
          <Box sx={{flexGrow: 1}}/>
          <ButtonBase onClick={handleClickListItem}>
            <Box sx={{display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'flex-end'}}>
              <Box sx={{display: 'flex', gap: '2px'}}>
                ...
                <Typography variant="body1" color="text.nonselect" sx={{fontWeight: 'medium', fontSize:'10px', alignSelf: 'center', textTransform: 'uppercase'}}>{chrome.i18n.getMessage('Data_from')}</Typography>
              </Box>
      ...
      <ResponsiveContainer width='100%' aspect={8.0/4.0}>
        ...
        >
          ...
          <YAxis orientation="right" domain={['auto', 'auto']} dataKey="price" tickFormatter={(item) =>{return '$'+ item } }/>
          ...
          <Area type="monotone" dataKey="price" stroke="#41CC5D" fillOpacity={1} fill="url(#colorUv)" />
        </AreaChart>
      </ResponsiveContainer>
      ...
      >
        {priceProviders.map((item, index) => (
          <MenuItem selected={index === selectedIndex} onClick={(event) => handleMenuItemClick(event, index)} key={index}>
            {Provider(index)}
          </MenuItem>
        ))}
      </Menu>
