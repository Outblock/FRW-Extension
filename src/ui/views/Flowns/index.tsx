  ...
  const handleClaiming = async () => {
    ...
    wallet.flownsPrepare().then(async (res) => {
      ...
      wallet.flownsResponse(script, domainArray[0], flownsAddress, lilicoAddress).then(async (res) => {
        wallet.listenTransaction(res['txId'], true, chrome.i18n.getMessage('Domain__creation__complete'), `Your flowns domain ${domain} has been created. \nClick to view this transaction.`);
    ...
    <Box sx={{
      ...
      >
        ...
        <Box sx={{position:'relative', width: '100%',marginLeft:'45px', alignItems: 'center',}}>
          <CardMedia sx={{ width:'260px', height:'34px', borderRadius: '12px', }} image={flownsPBanner} />
        </Box>
      </Box>
      <Box sx={{display:'flex', alignItems: 'center', justifyContent:'center', padding:'18px'}}>
        <Box 
          sx={{
            width: '100%',
            height: '60px',
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', justifyContent: 'start', alignItems: 'flex-start', py:'17px'}}>
          <Box sx={{flex:1}}>
          ...
          <Typography
            ...
            color="text.primary"
          >
            {chrome.i18n.getMessage('Free__lilico__domain__help__you__find__each__other__easier__on__chain')}
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', justifyContent: 'start', alignItems: 'flex-start', paddingBottom:'17px'}}>
          <Box sx={{flex:1}}>
            <CheckCircleIcon color='#41CC5D' size={24}/>
          </Box>
          <Typography
            ...
            color="text.primary"
          >
            {chrome.i18n.getMessage('An__Inbox__system__will__help__you__send__tokens__and__NFT__to__your__friend__without__enable__vault')}
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', justifyContent: 'start', alignItems: 'flex-start',}}>
          ...
          <Typography
            ...
            color="text.primary"
          >
            {chrome.i18n.getMessage('Anonymous__mode__will__be__affected__others__can__find__your__address__by__your__domain')}
          </Typography>
        </Box>
      </Box>
      <Box sx={{flexGrow:1}}/>
