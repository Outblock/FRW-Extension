...
class GoogleDriveService {
  ...
  deleteUserBackup = async (username: string) => {
    ...
    const updateContent = this.encrypt(
      JSON.stringify(newBackups),
      this.AES_KEY
    );
    if (!this.fileId) {
  ...
  loadBackup = async (): Promise<DriveItem[]> => {
    ...
    const firstOutblockBackup = files.find(
      (file) => file.name === 'outblock_backup'
    );
  ...
  getFile = async (fileId: string) => {
    return await this.sendRequest(`drive/v3/files/${fileId}/`, 'GET', {
      alt: 'media',
    });
  };
